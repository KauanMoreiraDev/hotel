...
const CardServiços = ({width = "full", backgroundColor = "bg-slate-600", srcImage, titleFont, titleColor, titleLabel = "Titulo aqui",label = "escreva seu texto aqui", colorButoon = "bg-white" } : CardProps) => {
                        ...
                        <div className="flex flex-col items-start ">
                            ...
                            <div className={`${backgroundColor} w-full z-10  pt-24 -mt-20 p-7 rounded-b-3xl`}>
                                <p className={`${titleFont} text-5xl ${titleColor} mb-2 pb-1`}>{titleLabel}</p>
                                    <p className="mb-2 pb-1 text-lg">{label}</p>
                                <Button onClick={handleClick} label={"Saiba mais"} wSize="w-full" background={`${colorButoon}`} />
                            </div>
                        </div>
                    </div>
    );
