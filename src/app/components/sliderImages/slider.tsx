        ...
        {/* h-[70vh] */}
            <div className="flex transition-transform ease-in-out duration-500" style={{ transform: `translateX(-${current * 100}%)` }}>
                {numberImagens.map((photos, index) => (
                    <Image key={index} src={photos.src} alt={`Slide ${index}`} width={1500} height={1500} className={`w-full ${constSrc === "photosChale" ? sizeQuarto : "h-96"} object-cover flex-shrink-0`} />
                ))}
            </div>
